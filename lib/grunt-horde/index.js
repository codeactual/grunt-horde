...
GruntHorde.prototype.kill = function(key) {
  ...
  if (parts.length === 1) {
    ...
    if (section === 'initConfig') {
      parts.shift();
      targetKey = parts.slice(-1)[0];
      delete teaProp.get(initConfig, parts.slice(0, -1).join('.'))[targetKey];
    } else {
      targetKey = parts.slice(-1)[0];
      delete teaProp.get(this.config, parts.slice(0, -1).join('.'))[targetKey];
    }
  }
  return this;
};
...
GruntHorde.prototype.learn = function(key) {
  ...
  if (section === 'initConfig') {
    if (parts.length === 1) {
      ...
      parts.shift();
      return teaProp.get(this.grunt.config.getRaw(), parts.join('.'));
    }
  } else {
    return teaProp.get(this.config, key);
  }
};
...
GruntHorde.prototype.configuredDemand = function(source, context, section, key, val) {
  ...
  var mode = '';
  var freezeKey = section + '.' + key;
  var parts = key.split('.');

  key = parts.join('.').replace('initConfig.', '');

  if (section === 'initConfig') {
    teaProp.set(this.grunt.config.getRaw(), key, val);
  } else {
    teaProp.set(this.config, key, val);
  }
...
GruntHorde.prototype.createModuleContext = function(file) {
  var context = {};
  context.kill = this.kill.bind(this);
  context.learn = this.learnInModule.bind(this);
  context.t = this.t.bind(this);

  var basename = path.basename(file, '.js');
  if (/initConfig/.test(file)) {
    context.demand = this.configuredDemand.bind(this, file, context, 'initConfig');
  } else {
    context.demand = this.configuredDemand.bind(this, file, context, basename);
  }
...
GruntHorde.prototype.learnInModule = function(key) {
  ...
  if ('undefined' === typeof value) {
    value = teaProp.get(this.config, key);
  }
  return value;
};
