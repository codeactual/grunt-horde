...
GruntHorde.prototype.attack = function() {
  ...
  this.grunt.initConfig(mergeDeep({}, this.config.initConfig, this.grunt.config.getRaw()));
...
GruntHorde.prototype.demand = function(key, val) {
  return this.sourcedDemand('Gruntfile', key, val);
};
 ...
 * @api private
 */
GruntHorde.prototype.sourcedDemand = function(source, key, val) {
  teaProp.set(this.grunt.config.getRaw(), key, val);
  this.grunt.event.emit('grunt-horde:demand', source, key, val);
  return this;
};
...
GruntHorde.prototype.createModuleContext = function(file) {
  return {
    ...
    demand: this.sourcedDemand.bind(this, file),
    t: this.t.bind(this)
  };
};
