...
GruntHorde.prototype.attack = function() {
  ...
  grunt.initConfig(this.config.initConfig);
...
GruntHorde.prototype.getConfig = function(key) {
  return teaProp.get(grunt.config.getRaw(), key);
};
...
GruntHorde.prototype.setConfig = function(key, val) {
  teaProp.set(grunt.config.getRaw(), key, val);
  grunt.event.emit('grunt-horde:set-config', key, val);
  return this;
};
...
GruntHorde.prototype.createModuleContext = function() {
  return {
    config: clone(grunt.config.getRaw()),
    path: path,
    shelljs: shelljs,
    t: this.t.bind(this)
  };
...
GruntHorde.prototype.require = function(name) {
  return require(this.resolveRequire(name)).call(this.createModuleContext(), grunt);
};
...
GruntHorde.prototype.t = function() {
  return grunt.template.process.apply(grunt, arguments);
};
