 ...
 * @api private
 */
GruntHorde.prototype.resolveRequireName = function(name) {
  if (/^\.\//.test(name)) {
    name = this.cwd + '/' + name;
  } else if (/^[^\/]/.test(name)) {
...
GruntHorde.prototype.require = function(name) {
  return require(this.resolveRequireName(name)).apply(
...
GruntHorde.prototype.requireDir = function(name) {
  var config = shelljs.ls(this.resolveRequireName(name) + '/*.js').reduce(
...
GruntHorde.reduceDirToConfig = function(memo, file) {
  if ('index.js' === file) { // Let it hold top-level keys
    ...
    memo.categorized = mergeDeep(memo.categorized, this.require(file));
  }
  return memo;
};
