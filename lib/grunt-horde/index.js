...
GruntHorde.prototype.configuredKill = function(source, context, section, key) {
  ...
  if (source !== 'Gruntfile' && this.frozenConfig[key]) {
    this.grunt.event.emit('grunt-horde:kill', source, section, key);
    return context;
  }
  ...
  if (parts.length === 1) {
    ...
    if (section === 'initConfig') {
      var targetObj = teaProp.get(initConfig, targetObjPath);
      if (targetObj) { delete targetObj[targetKey]; }
    } else {
      delete teaProp.get(this.config[section], targetObjPath)[targetKey];
    }
  }
