...
GruntHorde.prototype.configuredKill = function(source, context, section, key) {
  ...
  if (parts.length === 1) {
    ...
    if (section === 'initConfig') {
      targetObj = teaProp.get(initConfig, targetObjPath);
      if (targetObj) { delete targetObj[targetKey]; }
    } else {
      targetObj = teaProp.get(this.config[section], targetObjPath);
      if (targetObj) { delete targetObj[targetKey]; }
    }
  }
...
GruntHorde.prototype.resolveRequire = function(name) {
  ...
  if (/^lin/i.test(os)) {
    re.relative = /^\.\//; // Begins with `./`
    re.absolute = /^[^\/]/; // Does not begin with `/`
  } else if (/^win/i.test(os)) {
    re.relative = /^\.\\/; // Begins with `.\`
    re.absolute = /^(?![a-zA-Z]:[\/\\])/; // Ex. does not begin with `C:\`
  }

  if (re.relative.test(name)) {
    name = path.normalize(this.cwd + path.sep + name);
  } else if (re.absolute.test(name)) {
    name = this.cwd + path.sep + 'node_modules' + path.sep + name;
  }
